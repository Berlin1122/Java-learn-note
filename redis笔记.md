...
1. always:每次事件循环都将aof_buf缓冲区中的所有内容写入AOF文件，并且同步AOF文件。安全性最高。
2. everysec:每次事件循环都将aof_buf缓冲区中的所有内容写入AOF文件,并且每隔一秒就进行一次同步。安全性较高。
3. no:每次事件循环都将aof_buf缓冲区中的所有内容写入AOF文件，由操作系统决定同步时间。

（三）AOF重写：通过``BGREWRITEAOF``命令，可以对AOF文件进行重写。解决了自动追加写AOF文件导致，文件体积增大，载入时间长的问题。AOF重写会根据当前的数据库状态，对于每个键值对，使用更少的写命令记录数据库内容。
